...
def run_sweep() -> list[SweepResult]:
    ...
    print(
        f"Dataset loaded — train: {len(train_dataset.features):,} samples ({n_fraud_train} fraud) | "
...
def print_table(results: list[SweepResult]) -> None:
    ...
    print(
        ...
        f"epochs={best.epochs}  threshold={best.threshold:.2f}  "
...
def main() -> None:
    ...
    print(f"  Threshold sweep: {THRESHOLDS} (no retraining)")
    print("=" * 60 + "\n")
